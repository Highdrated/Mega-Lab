...
const PH = {
  ...
  mac: { help: "MAC address (aa:bb:cc:dd:ee:ff)", validate: t => /^([0-9a-fA-F]{2}:){5}[0-9a-fA-F]{2}$/.test(t), complete: () => [] },
...
const SWITCH_CFG_SPECS = [
  ...
  ["interfaces irb unit <unit:unit> family inet vrrp-group <group:unit> virtual-address <address:ip>", { kind:"value", help:"VRRP: share one gateway address between switches — the master answers, a backup takes over (real boxes nest this under the address itself)" }],
  ["interfaces irb unit <unit:unit> family inet vrrp-group <group:unit> priority <priority:unit>", { kind:"value", help:"Higher priority wins mastership (default 100)" }],
  ...
  ["access address-assignment pool <pool:word> family inet host <hostname:word> hardware-address <mac:mac>", { kind:"value", help:"Static reservation: this client (by MAC) always gets its named address" }],
  ["access address-assignment pool <pool:word> family inet host <hostname:word> ip-address <address:ip>", { kind:"value", help:"The fixed address for that reserved host" }],
